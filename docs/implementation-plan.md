...
- [x] （ユーザー）Cloudflareアカウントを作成する（既にあれば不要）
- [x] （ユーザー）ローカルに`wrangler`をインストールし、`wrangler login`でブラウザ認証する
...
- [x] （ユーザー）`.dev.vars`に`GEMINI_API_KEY`・`GITHUB_TOKEN`・`WORKER_PIN`・`AUTH_TOKEN_SECRET`のローカル用の値を記入した（`WORKER_PIN`は`1122`を維持。当初の`GEMINI_API_KEY`・`GITHUB_TOKEN`は無効だったため、新規発行の値に差し替え済み）
- [x] （ユーザー）Gemini APIキーが有効な「Auth key」であることを確認した（当初の値は無効だったため新規発行）
...
- [x] Cloudflare Workersでフロントエンド（静的ファイル）が表示され、PIN入力→認証Cookie取得の流れが動作する（本番URLで確認済み）
- [x] フロントエンドのコードに正解PINが含まれていない
