...
- **注意**：現行PINは既にPublicリポジトリ`fukuchan-app`のGitHub Pages配信コード内で公開されており、既に漏洩している前提で扱う。移行時は`/auth`方式への変更だけでなく、**PIN自体を新しい値へローテーションする**（`design.md` 5章）
...
- PINが現行値（既にPublicリポジトリで公開済み）から新しい値にローテーションされている
