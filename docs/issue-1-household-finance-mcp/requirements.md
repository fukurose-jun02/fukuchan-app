...
### 運用フォールバック：Option 2 手動CSV

自動同期の可否を判断するまでの暫定経路として、現行の月次CSVまたはMoney Forwardの「収入・支出詳細」CSVをローカルCLIから取り込めるようにする。月次CSVの基本列は`年月・カテゴリ・金額・メモ`、詳細CSVでは`日付・金額・大項目`を集計に使い、`内容・保有金融機関・メモ・ID`などは集計・SQL・ログへ出さない。月次CSVの`方向`列がある場合は収入・支出を分離し、ない場合は支出として扱う。詳細CSVは金額の符号から収入・支出を判定し、振替・対象外行は除外する。

この経路は`monthly_only`として扱い、日次行を推測・生成しない。したがって月次照会とカテゴリ照会は提供できるが、同期間比較はFR-12に従って`unsupported_granularity`相当で拒否する。実CSVをremote D1へ投入するには、合計一致と出力内容を確認したうえで別途承認する。
